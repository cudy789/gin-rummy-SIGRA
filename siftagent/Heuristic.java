...
//        lowCardWeight = .2 * ((double)1/GinRummyUtil.getDeadwoodPoints(c)); // the lower the card, the better (even if it doesn't reduce deadwood by much)
//        if (dist == 2){ // only consider this if adding this card would put us at most 1 card away from a meld
//            newMeldLaterWeight = .4 * ((double)1/GinRummyUtil.getDeadwoodPoints(c)); // favor cards with lower value
//        }
//
//
...
//            return MAXVALUE-.02;
//        } else if (dist <= -1 && dist > -100){ // more than 1 card goes into this meld, if we can remove any other card we can get rid of this one
//            return MAXVALUE - .01;
//        }
//        // Now for the actual qualitative to quantitative heuristic magic
//
...
//        double newMeldLaterWeight = MINVALUE; // 40% of our total weight
//
//        deadwoodWeight = .4 * ((double) 1/deadwoodPoints); // This formula takes 40% of a value between 1/10 -> 1/1. The lower the card, the better
//        lowCardWeight = .2 * ((double)1/GinRummyUtil.getDeadwoodPoints(c)); // the lower the card, the better (even if it doesn't reduce deadwood by much)
//        if (dist <= 2){ // only consider this if adding this card would put us at most 1 card away from a meld
//            newMeldLaterWeight = .4 * ((double)1/GinRummyUtil.getDeadwoodPoints(c)); // favor cards with lower value
//        }
//        retValue = (deadwoodWeight + lowCardWeight + newMeldLaterWeight) * .97; // card does not go into a meld, prefer to remove these rather than cards that do go into a meld
