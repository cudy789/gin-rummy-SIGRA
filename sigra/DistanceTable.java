...
//    }
//
//    // For each `suit` x `rank` x `length`, this matrix stores the distance we are from a straight
// with that
//    // `length` in that `suit` starting with that `rank`, that is, the number of cards we must
// draw to
//    // complete it. Straights which are impossible because they contain a discarded card are
// marked with the
//    // sentinel `MELD_IMPOSSIBLE`, and straights which we hold in are hand are marked with zero,
// conveninetly
//    // aliased `HAVE_THIS_MELD`.
//    private final int[][][] straight_distances = new
// int[Card.NUM_SUITS][Card.NUM_RANKS][SiftAgent.HAND_SIZE];
//
//    // For each `rank` x `n`, this matrix stores the distance we are from an `n`-of-a-kind match
// of that
//    // `rank`.
//    //
//    // TODO: figure out some way to represent the number of available cards which complete a given
//    // 3-of-a-kind. If we have `2C` and `2S`, `match_distances[2 - 1][3 - 1]` is 1 as long as
// either `2H` or
//    // `2D` or both are still in the deck.
//    private final int[][] match_distances = new int[Card.NUM_RANKS][Card.NUM_SUITS];
//
//    // The distance (i.e. number of required cards) to complete a meld which is entierly in our
// hand is zero.
//    public static final int HAVE_THIS_MELD = 0;
//    // Melds which cannot be completed because they contain cards we've seen discarded are maked
// with this
//    // sentinel.
//    public static final int MELD_IMPOSSIBLE = -100;
//    // The smallest melds that are actually worth points (or rather, not worth points) have 3
// cards.
//    public static final int SMALLEST_MELD = 3;
//
//    // Some straights are impossible because there just aren't that many cards. For example, you
// can't build a
//    // straight of 11 cards because you only get 10 cards in hand, and you can't build a 5-card
// straight
...
//    }
//
//    // Before you've seen any cards, you're `n` away from each run-of-`n`, except that some of
// them are
//    // impossible.
...
//                            = straight_possible(rank, straight_len) ? straight_len :
// MELD_IMPOSSIBLE;
//                }
//            }
//        }
...
//    //
//    // Note that at any given time, exactly one match depends on a given card: if this is the
// first card of
//    // that rank, the 4-of-a-kind is now impossible, but the 3-of-a-kind remains available. If
// this is the
//    // second, the 3-of-a-kind is now impossible, but the 2-of-a-kind is available (although
// useless).
...
////                // If the 4-of-a-kind of `rank` is still possible, mark it impossible.
...
//    }
//
//    // Returns the number of cards not in our hand required to make any straight containing
// `card`.
//    //
//    // See the comment on `distance_from_nearest_meld` for some nuance regarding what that value
// means.
...
//                numImpossible +=1;
//                if(numImpossible > 1){ // you can still get a 3 of a kind with 1 card marked
// impossible
...
//            }
//        }
//        if (dist > 0) dist -=1; // for the 3 card match, dist will be -1 if you have 4 of a kind
// in your hand!
...
//    //
//    // This means slightly different things depending on whether `card` is already in our hand or
// not.
//    //
//    // If `card` is already in our hand, then zero means it is part of a complete meld; one means
// that we
//    // need to draw one card to make the nearest meld, and so forth.
//    //
//    // If `card` is not in our hand, then zero means that we already have a complete meld in our
// hand to which
//    // `card` can be added; one means that `card` completes a meld in our hand; two means that we
// would need
//    // another card in addition to `card` to complete a meld, and so forth.
//    public int distance_from_nearest_meld(Card card) {
//        return Integer.min(distance_from_nearest_match(card),
// distance_from_nearest_straight(card));
//    }
//
...
//            System.out.println("does " + c + " have 2 different kinds of melds? " +
// distance_from_nearest_match(c) + "==" + distance_from_nearest_straight(c) + "?? " +
// (distance_from_nearest_match(c) == 0 && distance_from_nearest_straight(c) == 0));
//            if (distance_from_nearest_match(c) == 0 && distance_from_nearest_straight(c) == 0) {//
// card is in an unresolved straight and meld
//                System.out.println(c + " has 2 different kinds of melds, sorting out the
// situation");
////                ArrayList<ArrayList<ArrayList<Card>>> setOfMelds =
// GinRummyUtil.cardsToBestMeldSets(player.my_hand);
...
//                if (SiftAgent.is_straight_meld(c_meld)) { // if the best meld for it to be in is a
// straight, then remove it from the matches
//                    make_matches_further(c);
//                    System.out.println("putting " + c + " into a straight instead of a match");
//                } else {
//                    make_straights_further(c); // if the best meld is a match, remove it from the
// straights
...
//
//
//    // Mark each of the melds containing `card` as being impossible because that card is in the
// discard pile.
...
//                 ((rank + straight_length) < Card.NUM_RANKS) && (straight_length <=
// SiftAgent.HAND_SIZE);
//                 straight_length++) {
...
////        }
//    }
//    // Increase the distance of each straight containing `card` by one, used on a card that was in
// a straight but now in a match
...
//    }
//
//    // Increase the distance of each match containing `card`, used on a card that was in a match
// but now in a straight
